    ...
    for level in tau:
        ...
        for max_depth in [4, 5, 6]:
            for n_estimators in [50, 100, 200]:
                ...
                model_lgb = lgb.LGBMRegressor(objective = 'quantile', alpha = level, num_leaves  = 2 ** max_depth - 1,
                                              max_depth = max_depth, n_estimators = n_estimators, random_state  = set_seed)
                model_lgb = model_lgb.fit(train_data_s[var], train_data_s[y])
                valid_loss = mean_pinball_loss(valid_data[y], model_lgb.predict(valid_data[var]), alpha = level)
                if valid_loss < best_valid_loss:
                    best_valid_loss = valid_loss
                    best_model_lgb = {'max_depth':max_depth, 'n_estimators':n_estimators}
        
        model_lgb = lgb.LGBMRegressor(objective = 'quantile', alpha = level, num_leaves = 2 ** best_model_lgb['max_depth'] - 1,
                                      max_depth = best_model_lgb['max_depth'], n_estimators = best_model_lgb['n_estimators'],
                                      random_state  = set_seed)
        model_lgb = model_lgb.fit(train_data[var], train_data[y])
        quantiles[:,int(level*100-1)] = model_lgb.predict(test_data[var])
