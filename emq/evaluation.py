...
# emq
    ...
    if not os.path.isfile(data_dir + "emq.csv"):
        with open(data_dir + "emq.csv", 'a') as file:
    ...
    if not os.path.isdir(data_dir + "emq/"):
        os.mkdir(data_dir + "emq/")
    for t in range(len(Qlist)):
        results = pd.DataFrame(Qlist[t], columns = tau)
        results['true'] = test_data[y].tolist()
        results.to_csv(data_dir + "emq/" + "emq_{}_{}_{}_{}_{}_{}.csv".format(set_seed, T, C, str(dims_i), str(dims_t), t), index = False)
