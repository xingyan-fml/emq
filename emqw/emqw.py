...
def pinball_loss(Q, y, tau, weights = 1):
    error = y - Q
    error1 = torch.multiply(error, tau)
    error2 = torch.multiply(error, tau - 1)
    loss = torch.mean(torch.multiply(torch.maximum(error1, error2), weights))
    return loss

def penalty_pinball_loss(Q, y, params, C, tau, weights = 1):
    loss1 = pinball_loss(Q, y, tau, weights)
...
def fit_emq(data, var, y, tau, T, C, initial_dims, tuning_dims, num_epochs, batch_size, lr, train_prop, set_seed):
    ...
    error = np.random.randn(1000000, 1) - norm.ppf(tau).reshape(1, -1)
    error1 = np.multiply(error, tau.reshape(1, -1))
    error2 = np.multiply(error, tau.reshape(1, -1) - 1)
    tau_weights = 1 / np.mean(np.maximum(error1, error2), axis = 0)
    tau_weights = tau.size * tau_weights / np.sum(tau_weights)
    tau_weights = torch.tensor(tau_weights, dtype = torch.float32).reshape(1, -1)
    ...
    for epoch in range(num_epochs):
        ...
        for x_batch, y_batch in train_iter:
            model[0].train()
            loss = pinball_loss(model[0](x_batch), y_batch, tau_t, tau_weights)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            ...
            model[0].eval()
            with torch.no_grad():
                valid_loss = pinball_loss(model[0](x_valid), y_valid, tau_t, tau_weights).detach().numpy().item()
...
#==============================================================================
    ...
    for t in range(1, T + 1):
        ...
        with torch.no_grad():
            ...
            best_valid = pinball_loss(out, y_valid, tau_t, tau_weights).detach().numpy().item()
        ...
        for epoch in range(num_epochs):
            ...
            for x_batch, y_batch, oldQ_batch, diff_batch in train_iter:
                ...
                loss = penalty_pinball_loss(out, y_batch, params, C, tau_t, tau_weights)
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
                ...
                with torch.no_grad():
                    out, params = model[t](x_valid, diff_valid, oldQ_valid)
                    valid_loss = pinball_loss(out, y_valid, tau_t, tau_weights).detach().numpy().item()
                batch_valids.append(valid_loss)
                if valid_loss < best_valid:
                    best_valid = valid_loss
                    best_dict = copy.deepcopy(model[t].state_dict())
